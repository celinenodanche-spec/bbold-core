...
export async function POST(req: Request) {
  try {
    const { platform, format, idea, template, refId, tools, style, brand: brandSlug } = (await req.json()) as {
      platform?: Platform; format?: Format; idea?: string; template?: string; refId?: string; tools?: string[]; style?: string; brand?: string;
    };
    ...
    const post = await addPost({ platform, format, idea: idea.trim(), template, refId, tools, style, brandSlug: brand?.slug, result });
